...
for it in tqdm(range(epoch)):
    ...
    for truth_X, mask, data_X in train_loader:
        ...
        Imputer_loss = loss(truth=truth_X, mask=mask, data=data_X)[0]
        ...
        prediction = loss(truth=truth_X, mask=mask, data=data_X)[1]
    ...
    print('Train_loss: {:.4}'.format(np.sqrt(Imputer_loss.item())))
...
with torch.no_grad():
    ...
    for truth_X, mask, data_X in test_loader:
        ...
        MSE, prediction =  loss(truth=truth_X, mask=mask, data=data_X)
        imputed_data = impute_with_prediction(truth_X, mask, prediction)
